# 2D MOSFET UTBB FDSOI (n type, 60 nm technology)
# conventional well process (p-type doping for both Si substrate and Si overlay)
# ----------------------------------------------------
math coord.ucs
...
struct tdr=n@node@_03_FDSOI_soi; # silicon overlay deposition and creation of the complete SOI wafer
...
# Gate oxidation
# ----------------------------------------------------
diffuse temperature= 700<C> time= 0.5<min> O2
struct tdr=n@node@_04_FDSOI_tox;

# Poly gate deposition (60 nm channel length)
# ----------------------------------------------------
deposit material= {PolySilicon} type= anisotropic time= 1 rate= {0.065}
struct tdr= n@node@_05_FDSOI_gate_dep;
...
etch material= {PolySilicon} type= anisotropic time= 1 rate= {0.067} \
  mask = gate_mask
struct tdr= n@node@_06_FDSOI_gate_poly_etch;

# GOX pattern/etch 
# anisotropic etching, GOX thickness (1.5 nm) + tolerance (0.5 nm) to avoid residuals
# ----------------------------------------------------
etch material= {Oxide} type= anisotropic time= 1 rate= {0.002}
struct tdr= n@node@_07_FDSOI_gate_oxide_etch; 

# Poly reoxidation
# ----------------------------------------------------
diffuse temperature= 850<C> time= 0.5<min> O2
struct tdr= n@node@_08_FDSOI_poly_reox;

# LDD implantation and diffusion
# ----------------------------------------------------
refinebox Silicon min= {-0.081 0.025} max= {-0.065 0.04} xrefine= {0.003 0.005} \
                             yrefine= {0.002} add
grid remesh

implant Arsenic dose= 4e13<cm-2> energy= 0.01<keV> tilt= 0 rotation= 0
struct tdr= n@node@_09_FDSOI_LDD_implant;

diffuse temperature= 1050<C> time= 0.1<s>
struct tdr= n@node@_10_FDSOI_LDD_diffuse;

# Nitride spacers
# ----------------------------------------------------
deposit material= {Nitride} type= isotropic time= 1 rate= {0.02}
struct tdr= n@node@_11_FDSOI_spacer_dep;

etch material= {Nitride} type= anisotropic time = 1 rate= {0.025} \
  isotropic.overetch= 0.01
struct tdr= n@node@_12_FDSOI_spacer_etch;

etch material= {Oxide} type= anisotropic time= 1 rate= {0.002}
struct tdr= n@node@_13_FDSOI_spacer_ox_removal;

# Source/drain implantation
# ----------------------------------------------------
refinebox Silicon min= {-0.076 0.05} max= {-0.065 0.15} xrefine= {0.003 0.005} \
						      yrefine= {0.002} add
grid remesh

implant Arsenic dose= 7e13<cm-2> energy= 4<keV> \
        tilt= 7<degree> rotation= -90<degree>
struct tdr= n@node@_14_FDSOI_SD_implant;

# Final RTA
# ----------------------------------------------------
diffuse temperature= 1050<C> time= 5.0<s>
struct tdr= n@node@_15_FDSOI_SD_diffuse;

# RMG
# ----------------------------------------------------
mask name= rmg_mask left=-1 right= 30<nm> negative
etch material= {PolySilicon} type= anisotropic time= 1 rate= {0.067} \
  mask = rmg_mask
struct tdr= n@node@_16_FDSOI_dummy_gate_etch;

deposit material= {Tungsten} type= polygon polygon= {-0.081 0 -0.081 0.03 -0.147 0.03 -0.147 0} \
  mask = rmg_mask
...
refinebox Silicon min= {-0.081 -0.04} max= {-0.065 -0.025} xrefine= {0.003 0.005} \
                              yrefine= {0.002}
refinebox Silicon min= {-0.076 -0.15} max= {-0.065 -0.05} xrefine= {0.003 0.005} \
						      yrefine= {0.002}
refinebox Silicon min= {-0.081 0.025} max= {-0.065 0.04} xrefine= {0.003 0.005} \
                              yrefine= {0.002}
refinebox Silicon min= {-0.076 0.05} max= {-0.065 0.15} xrefine= {0.003 0.005} \
						      yrefine= {0.002}
refinebox Silicon min= {-0.076 -0.02} max= {-0.065 0.02} xrefine= {0.005 0.005} \
						      yrefine= {0.005}
grid remesh
